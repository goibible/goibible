#!/usr/bin/env python3
"""Stage 2: render one 1080p PNG per verse for a Proverbs chapter.

For each verse we draw the ENTIRE chapter as a scrolling text column with that
verse highlighted (bold + background band) and vertically centered in the frame.
Consecutive frames differ only by a small scroll offset, so played back they read
as smooth scrolling — the same feel as the ESV reading videos.

Frame i is shown for exactly verse i's audio window (see render_video.py), so the
highlighted verse on screen always matches the verse being spoken.

Output: work/proverbs_<cc>/frames/<cc>_<vv>.png
"""
...
EDITION_LINE = "GOI Bible · English · Proverbs"

# Try a clean serif; fall back to a default PIL font if none found.
FONT_CANDIDATES = [
...
def wrap(text: str, font: ImageFont.FreeTypeFont, max_w: int) -> list[str]:
    """Word-wrap text to max_w pixels, preserving intended punctuation spacing."""
    words = text.split()
    if not words:
        return [""]
    lines: list[str] = []
...
def build_rows(verses: list[dict]) -> list[dict]:
    """Return a list of draw rows:

    {'kind': 'heading'|'attribution'|'verse_start'|'verse_cont',
     'verse': int|None, 'text': str}
    verse_start rows carry the verse number gutter; verse_cont rows don't.
    We also attach row index ranges per verse for vertical centering.
    """
    head_font = load_font(HEADING_SIZE)
    verse_font = load_font(VERSE_SIZE)

    rows: list[dict] = [
        {"kind": "heading", "verse": None, "text": "The Proverbs"},
        {"kind": "attribution", "verse": None, "text": EDITION_LINE},
    ]

    for v in verses:
        if v.get("verse") == 1:
            rows.append(
                {"kind": "heading", "verse": None, "text": f"Chapter {v['chapter']}"}
            )
    ...
    # Compute vertical positions: heading/attribution rows get line height,
    # verse rows get line height, plus PARA_GAP before each verse_start.
...
def verse_row_range(rows: list[dict], verse: int):
    """(first_top, last_bottom) of the rows belonging to *verse*."""
...
def render_frame(rows: list[dict], total_h: int, active_verse: int, out_path: Path) -> None:
    ...
    # Frame shows a window [offset, offset+FRAME_H]; keep it within the column.
    top_margin = 80
    bottom_margin = 80
    ...
    for row in rows:
        ...
        if row["kind"] == "heading":
            ...
            x = MARGIN_L
            if is_active:
                # Background band across the whole text width for the active verse.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render highlight frames for a Proverbs chapter.")
    parser.add_argument("--chapter", type=int, required=True)
    parser.add_argument("--resume", action="store_true", help="Skip frames that already exist")
    args = parser.parse_args()

    work = Path("work") / f"proverbs_{args.chapter:02d}"
    timeline = json.loads((work / "timeline.json").read_text(encoding="utf-8"))
    verses = timeline["verses"]
    if not verses:
        raise SystemExit(f"No verses in timeline for chapter {args.chapter}")

    rows, _ypos, total_h = build_rows(verses)
    frame_dir = work / "frames"
    frame_dir.mkdir(parents=True, exist_ok=True)

    # Persist a layout summary so render_video can map verse -> frame file.
    for v in verses:
        vv = v["verse"]
    ...
    print(f"chapter {args.chapter}: {len(verses)} frames -> {frame_dir}", flush=True)
