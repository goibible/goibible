...
There is **one Greek verse spine by default** (Textus Receptus, Stephanus 1550)
and a **disambiguation layer** that was built once, language-neutrally. Your job
for a new language is mostly **filling in renderings**, not re-deciding meaning.
Meaning-changing textual variants are governed by the language-neutral policy in
`staging/textual_policy/nt_textual_policy.csv`.

```
  Greek TR1550 (raw text + Strong's morphology)        <- default verse spine, frozen
  textual_policy overrides                             <- meaning-changing variant decisions
        │
        ...
        ↓
  translated verses: one file per verse in GOI_Bible_<BCP47>/
        ↓
...
| `GOI_Bible_<BCP47>/` | **you create this** — your translated output, using the BCP 47 language tag suffix |
...
For new output directories, use the BCP 47 language tag as the suffix:
`GOI_Bible_vi`, `GOI_Bible_es`, `GOI_Bible_zh-Hans`. Older completed corpora
may keep their historical descriptive names until a deliberate migration.
...
- It is **Textus Receptus**, not a modern critical text. TR1550 is the default
  verse spine, but meaning-changing variants follow
  `staging/textual_policy/nt_textual_policy.csv`.
- Bracket notation is language-neutral: `[]` marks conflicted ancient readings;
  `{}` marks supplemental traditional material. Romans 12:11 is not bracketed:
  translate it as "serving the Lord" in every language.
- Some TR readings modern Bibles omit are retained with notation instead of
  silently treated as normal mainline text (e.g. Mark 16:9-20, John 7:53-8:11,
  Acts 8:37, 1 John 5:7-8).
...
Let `L` be your language/rendering code (e.g. `es`, `fr`, `ar`, `sw`). Let
`BCP47` be the output dir suffix (e.g. `es`).
...
Create `GOI_Bible_<BCP47>/` with one file per verse (same filenames as
...
- `strongs_lang_renderings` + `sense_renderings` as your controlled vocabulary
  for nouns (query `bible_noun.sqlite3`),
...
GOI_Bible_<BCP47>` to canonicalize punctuation.

### Step 6 — Verify coverage

```
python3 verify_coverage.py --lang L --output-dir ../GOI_Bible_<BCP47>
python3 verify_coverage.py --lang L --output-dir ../GOI_Bible_<BCP47> --missing-only
```
...
python3 Bible_Noun_Extraction/falsefriend_sweep.py xref \
        --draft GOI_Bible_<BCP47> --lang L --refs <pd_ref_dir_1>,<pd_ref_dir_2>
