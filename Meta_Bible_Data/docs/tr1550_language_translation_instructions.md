...
**Master instructions for translating the Greek Textus Receptus (Stephanus
...
One frozen Greek verse spine (TR1550) + a **language-neutral disambiguation
layer** built once. Translating a new language is mostly *supplying words*, not
...
coordinates** and transfer to every language unchanged. Meaning-changing
textual variants are separately governed by
...
├── GOI_Bible_<BCP47>/            YOU CREATE THIS — your output, same filenames
...
└── Bible_Noun_Extraction/    the pipeline + database (git submodule)
...
`docs/…` and `Bible_Noun_Extraction/…` are repo-root-relative).
...
**Output directory convention:** for new editions, use the BCP 47 language tag
as the suffix: `GOI_Bible_vi`, `GOI_Bible_es`, `GOI_Bible_zh-Hans`. Older
finished corpora retain their historical descriptive names unless explicitly
migrated.

**Onboarding steps (per language L, output dir suffix `BCP47`):**
...
5. **Translate** the 7,957 verses *from the Greek* into `GOI_Bible_<BCP47>/`,
   ...
   structural (not source) references. `normalize_corpus.py --dir GOI_Bible_<BCP47>`.
6. **Coverage:** `verify_coverage.py --lang L --output-dir ../GOI_Bible_<BCP47>`;
   chase `--missing-only`.
...
python3 Bible_Noun_Extraction/falsefriend_sweep.py strongs \
    --draft GOI_Bible_<BCP47> --strongs 4982 \
...
**Source (do not "fix"):** TR1550 is the default verse spine and must not be
renumbered. Meaning-changing variants follow
`staging/textual_policy/nt_textual_policy.csv`: `[]` marks conflicted ancient
readings, `{}` marks supplemental traditional material, and unbracketed critical
overrides such as Romans 12:11 are translated from the preferred older reading.
Bracketed scribal **colophons** (2 Cor 13:14, Eph 6:24) are paratext —
translate them in brackets.
...
```bash
L=es; LANGUAGE=Spanish; BCP47=es        # example
...
sqlite3 Bible_Noun_Extraction/bible_noun.sqlite3 < ${L}_defaults_review.sql
...
python3 Bible_Noun_Extraction/translate_verses.py \
  --lang $L --language-name "$LANGUAGE" --output-dir GOI_Bible_$BCP47 \
...
cat Bible_Noun_Extraction/One_Directory_TR1550/044_ACT_009_029_TR1550.txt
```
