...
Expect **Textus Receptus readings** that modern Bibles omit or footnote. TR1550
is the default verse spine, but meaning-changing textual variants are governed by
`staging/textual_policy/nt_textual_policy.csv`. GOI notation uses `[]` for
conflicted ancient readings and `{}` for supplemental traditional material.

**Versification follows TR1550**, which can diverge from KJV/modern numbering.
Do not renumber against a modern Bible; handle textual differences through the
