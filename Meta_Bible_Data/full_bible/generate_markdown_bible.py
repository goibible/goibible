...
DB_PATH = META / "local_backups" / "GOI_bible.sqlite3"
OUTPUT_DIR = META / "full_bible"
...
SOURCE_DIRS = {
    "GOI_En": ROOT / "GOI_Bible" / "GOI_Bible_English",
    "GOI_Zh_Hant": ROOT / "GOI_Bible" / "GOI_Bible_Chinese_Hant",
    "GOI_Zh_Hans": ROOT / "GOI_Bible" / "GOI_Bible_Chinese_Hans",
}
...
def export_markdown(conn: sqlite3.Connection, edition: Edition, output_dir: pathlib.Path, overwrite: bool) -> pathlib.Path:
    ...
    verse_rows = conn.execute(
        """
        SELECT v.conical, v.book, v.chapter, v.verse
        FROM verses v
        WHERE v.edition_id = ?
    ...
    with output_path.open("w", encoding="utf-8", newline="\n") as fh:
        for conical, book, chapter, verse in verse_rows:
            ...
            filename_key = f"{conical:03d}_{book}_{chapter:03d}_{verse:03d}_{edition.edition_id}"
