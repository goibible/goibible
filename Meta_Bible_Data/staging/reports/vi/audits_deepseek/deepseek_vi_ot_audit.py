#!/usr/bin/env python3
"""Hebrew->Vietnamese OT translation-fidelity audit via DeepSeek-V4-Flash-0731
(DeepInfra, OpenAI-compatible API, config from repo-root .env).

Adapted from deepseek_vi_nt_audit.py (same directory) for the Old Testament:
Hebrew WLC source instead of Greek TR1550, translation_fidelity check only
(no Strong's-coverage / noun-consistency passes -- there is no Hebrew-side
Strong's DB wired up the way greek_noun.sqlite3 covers the NT).

Every flag still needs manual Hebrew/KJV verification before being trusted --
this tool is a candidate generator, not a verdict (same lesson learned on the
NT audits: even a well-calibrated model still needs every flag checked by hand).

Usage:
  python3 deepseek_vi_ot_audit.py               # full OT, resumable
  python3 deepseek_vi_ot_audit.py --book RUT     # one book (smoketest)
  python3 deepseek_vi_ot_audit.py --report       # build priority queue CSV only
...
def fidelity_prompt(hebrew: str, vietnamese: str) -> tuple[str, str]:
    system = (
        ...
        "different from, or missing something substantive that is present in, the Hebrew. "
        ...
        "Vietnamese verb with an intensifier ('chắc chắn', 'thật', 'hết lòng') instead of "
        "a literal doubled word -- that is correct, not an omission.\n"
        ...
        "rendered with Vietnamese reduplication ('ngàn ngàn', 'hai hai') -- this is a "
        "genuine, correct feature of Vietnamese, not a duplication bug.\n"
        "- Waw-consecutive/converted tense forms rendered with Vietnamese aspect markers "
        "('đã', 'sẽ', 'đang') or narrative word order instead of a literal tense gloss -- "
        "Vietnamese verbs are not inflected the way Hebrew is, infer aspect from context.\n"
        "- Construct-chain (smikhut) possessives rendered as a natural Vietnamese "
        "possessive phrase instead of a literal 'X of Y' gloss.\n"
        "- Hebrew parallelism (synonymous or antithetic parallel clauses, common in "
        "poetry/Psalms/Proverbs) where the Vietnamese preserves the same two-part "
        "structure and meaning even if word choice differs between the two halves.\n"
        "- Connective/appositive words with no separate Hebrew lexeme (e.g. 'là', "
        "'rằng', 'tức là') added for natural Vietnamese.\n"
        "- Any rendering you would still call 'a close/acceptable translation' after "
        "your own explanation -- if your explanation agrees the Vietnamese is basically "
...
def main() -> None:
    ...
    total = len(verses)
    if total == 0:
        sys.exit("No matching verses found (check --book and that GOI_vi/WLC files exist).")

    done_verses = errors = 0
    for i, (stem, code, ch, vs, hf, vf) in enumerate(verses, 1):
        ...
        system, user = fidelity_prompt(hebrew, vietnamese)
