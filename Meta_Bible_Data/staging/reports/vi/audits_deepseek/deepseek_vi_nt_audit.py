#!/usr/bin/env python3
"""Three-pass Greek->Vietnamese NT audit via DeepSeek-V4-Flash-0731
(DeepInfra, OpenAI-compatible API, config from repo-root .env).

Re-run of the NT fidelity check using DeepSeek instead of the local
gemma4-12b checker (see ../audits/gemma4_vi_nt_audit.py and
NT_FIDELITY_AUDIT_STATUS.md) -- DeepSeek smoketested with a near-zero
false-positive rate vs. gemma4-12b's 70-90%, so this should surface real
issues gemma4-12b's noise buried, and cross-check the earlier findings.
...
Usage:
  python3 deepseek_vi_nt_audit.py               # full NT, resumable
  python3 deepseek_vi_nt_audit.py --book PHM     # one book (smoketest)
  python3 deepseek_vi_nt_audit.py --report       # build priority queue CSV only
  python3 deepseek_vi_nt_audit.py --checks translation_fidelity
...
def fidelity_prompt(greek: str, vietnamese: str) -> tuple[str, str]:
    system = (
        ...
        "FLAG ONLY if a Vietnamese reader would come away believing something "
        ...
        "not errors:\n"
        "- Natural Vietnamese connectors/appositive markers with no separate Greek word "
        "(e.g. 'tức là', 'là', 'rằng') inserted to make the sentence read naturally.\n"
        "- Prepositions or particles rendered by Vietnamese word order or a possessive/"
        ...
        "same meaning as the Greek even if the surface grammar differs -- Vietnamese verbs "
        "are not inflected for tense/number/person the way Greek is, so this must be "
        "inferred from context/aspect markers ('đã', 'sẽ', 'đang'), not surface morphology.\n"
...
def strongs_prompt(greek: str, vietnamese: str, rows: list[dict]) -> tuple[str, str]:
    ...
    system = (
        ...
        "each with its expected sense, and the Vietnamese translation. Confirm "
        ...
        "the Vietnamese. Flag a lemma only if it is missing, its sense is distorted, or the "
        "Vietnamese adds a concept with no corresponding lemma.\n\n"
        "Prepositions and conjunctions are very often correctly rendered by Vietnamese word "
...
def noun_prompt(vietnamese: str, rows: list[dict]) -> tuple[str, str]:
    ...
    system = (
        ...
        "Vietnamese New Testament translation. You are given the Greek nouns "
        ...
        "noun, confirm the Vietnamese uses the correct name/title and does not drop, swap, "
        "or misidentify it. Where an established rendering is given, the Vietnamese should "
...
def iter_verses(book_filter: Optional[str]):
    for gf in sorted(GREEK.glob("*.txt")):
        ...
        ch = int(stem[8:11])
        vs = int(stem[12:15])
        vf = VI_DIR / f"{stem}{VI_EDITION_SUFFIX}"
        if not vf.exists():
            continue
