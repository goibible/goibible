#!/usr/bin/env python3
"""Three-pass Greek->Chinese (Traditional) NT audit via DeepSeek-V4-Flash-0731
(DeepInfra, OpenAI-compatible API, config from repo-root .env).

Mirrors gemma4_vi_nt_audit.py's structure and calibration (learned the hard
way on the Vietnamese NT pass -- see NT_FIDELITY_AUDIT_STATUS.md): the
fidelity prompt explicitly tells the model not to flag natural translation
choices, and to default to OK when in doubt. Every flag still needs manual
Greek/KJV verification before being trusted -- this tool is a candidate
generator, not a verdict.

Usage:
  python3 deepseek_zh_nt_audit.py               # full NT, resumable
  python3 deepseek_zh_nt_audit.py --book PHM     # one book (smoketest)
  python3 deepseek_zh_nt_audit.py --report       # build priority queue CSV only
  python3 deepseek_zh_nt_audit.py --checks translation_fidelity
...
DB = META_ROOT / "Bible_Noun_Extraction" / "greek_noun.sqlite3"
...
def get_strongs_rows(conn: sqlite3.Connection, book: str, ch: int, vs: int) -> list[dict]:
    rows = conn.execute("""
        ...
        LEFT JOIN strongs_lang_renderings slv
               ON slv.strongs_num = snt.strongs_num AND slv.lang = 'zh'
...
def get_noun_rows(conn: sqlite3.Connection, book: str, ch: int, vs: int) -> list[dict]:
    rows = conn.execute("""
        SELECT o.surface_form, n.lemma, c.category_code AS category,
               slv.rendering AS established_zh_rendering
        ...
        LEFT JOIN strongs_lang_renderings slv
               ON slv.strongs_num = lex.strongs_num AND slv.lang = 'zh'
...
def fidelity_prompt(greek: str, chinese: str) -> tuple[str, str]:
    system = (
        ...
        "the Greek. If you are not sure, or the difference is purely stylistic, mark OK. "
        "When in doubt, OK.\n\n"
        "Do NOT flag any of the following -- they are correct, idiomatic translation, "
        "not errors:\n"
        "- Natural Chinese connectors/appositive markers with no separate Greek word "
        "(e.g. 就是, 是, 說) inserted to make the sentence read naturally.\n"
        "- Prepositions or particles rendered by Chinese word order or a possessive/"
        "locative construction instead of a literal one-to-one gloss.\n"
        "- A relative clause, verb tense choice, or clause structure that conveys the "
        "same meaning as the Greek even if the surface grammar differs -- Chinese verbs "
        "are not inflected for tense/number/person the way Greek is, so this must be "
        "inferred from context/aspect markers (了, 要, 將), not surface morphology.\n"
        ...
        "the Chinese is basically right, the verdict must be OK, not FLAG.\n"
...
def strongs_prompt(greek: str, chinese: str, rows: list[dict]) -> tuple[str, str]:
    lines = []
    for r in rows:
        gloss = r["zh_rendering"] or r["english"] or r["lex_definition"] or "?"
    ...
    system = (
        ...
        "each with its expected sense, and the Chinese (Traditional) translation. Confirm "
        "every meaningful lemma (skip pure grammar words: articles, particles like "
        "δε/και/γαρ unless they carry real semantic weight) is represented correctly in "
        "the Chinese. Flag a lemma only if it is missing, its sense is distorted, or the "
        "Chinese adds a concept with no corresponding lemma.\n\n"
        "Prepositions and conjunctions are very often correctly rendered by Chinese word "
        "order or a natural connector instead of a separate preposition word -- that is "
        "NOT missing or distorted, do not flag it. If your own explanation says the "
        "rendering is 'acceptable' or 'close', the verdict must be OK, not FLAG. When in "
        "doubt, OK.\n\n"
        "Respond with JSON only, matching this schema:\n"
...
def noun_prompt(chinese: str, rows: list[dict]) -> tuple[str, str]:
    lines = []
    for r in rows:
        established = f" -> established Chinese rendering: {r['established_zh_rendering']}" \
            if r["established_zh_rendering"] else ""
    ...
    system = (
        ...
        "Chinese (Traditional) New Testament translation. You are given the Greek nouns "
        "extracted for this verse (person/place/God names, titles, and other nouns), some "
        "with an established project rendering. For each PERSON, PLACE, or GOD category "
        "noun, confirm the Chinese uses the correct name/title and does not drop, swap, "
        "or misidentify it. Where an established rendering is given, the Chinese should "
        "match it. Do not flag common nouns (category OTHER) unless they are a project "
        "theological term rendered inconsistently.\n\n"
        "Respond with JSON only, matching this schema:\n"
        f"{json.dumps(NounResult.model_json_schema())}"
    )
    user = f"CHINESE (Traditional): {chinese.strip()}\n\nGREEK NOUNS EXTRACTED:\n{noun_block}"
    return system, user
...
def call_check(client: OpenAI, schema: type[BaseModel],
...
def main() -> None:
    ...
    ap.add_argument("--book", help="limit to one book e.g. PHM")
    ...
    total = len(verses)
    if total == 0:
        sys.exit("No matching verses found (check --book and that GOI_Zh_Hant files exist).")

    done_verses = errors = 0
    for i, (stem, code, ch, vs, gf, zf) in enumerate(verses, 1):
        ...
        if "translation_fidelity" in active_checks and not check_is_valid(record, "translation_fidelity"):
            system, user = fidelity_prompt(greek, chinese)
        ...
        if "strongs_coverage" in active_checks and not check_is_valid(record, "strongs_coverage"):
            ...
            system, user = strongs_prompt(greek, chinese, rows)
        ...
        if "noun_consistency" in active_checks and not check_is_valid(record, "noun_consistency"):
            ...
            system, user = noun_prompt(chinese, rows)
