#!/usr/bin/env python3
"""Integrity gate for the Chinese NT corpora.

Mirrors validate.py (English gate) — run after any edit to the Chinese corpus or DB.

  python3 validate_zh.py
  python3 validate_zh.py --variant hant
  python3 validate_zh.py --variant hans
...
Checks (each prints PASS/FAIL):
  ...
  COVERAGE  Chinese noun coverage >= 95% (delegates to generic verify_coverage.py)
"""
...
RAW  = ROOT / "Reference_Bible" / "Greek_Bible_TR1550" / "One_Directory_TR1550"
...
VARIANTS = {
    "hant": ("Traditional Chinese", ROOT / "GOI_Bible" / "GOI_Bible_Chinese_Hant", "GOI_Zh_Hant"),
    "hans": ("Simplified Chinese", ROOT / "GOI_Bible" / "GOI_Bible_Chinese_Hans", "GOI_Zh_Hans"),
}
...
def validate_variant(label, goi_dir, filename_suffix):
    ...
    files = sorted(goi_dir.glob("*.txt"))
    check("file count == 31102", len(files) == 31102, f"got {len(files)}")
    bad_re = rf"\d{{3}}_[A-Z0-9]{{3}}_\d{{3}}_\d{{3}}_{re.escape(filename_suffix)}\.txt"
    bad = [f.name for f in files if not re.fullmatch(bad_re, f.name)]
    ...
    print(f"=== NOUN COUNT (canonical to raw TR1550): {label} ===")
    a = dict(cur.execute(
    ...
    print(f"=== CHINESE COVERAGE: {label} ===")
    result = subprocess.run(
        [
            sys.executable,
            str(NIM / "verify_coverage.py"),
            "--lang",
            "zh",
            "--output-dir",
            str(goi_dir),
    ...
    m_rate    = re.search(r"coverage:\s+([\d.]+)%", result.stdout)
...
def main():
    ...
    parser.add_argument("--variant", choices=("hant", "hans", "both"), default="both")
    args = parser.parse_args()

    selected = ("hant", "hans") if args.variant == "both" else (args.variant,)
