#!/usr/bin/env python3
"""Translate KJV-addressed WLC Hebrew OT verses into English, one verse per file.

Mirrors the proven NT pipeline (Bible_Noun_Extraction/translate_verses.py):
read Hebrew source text, resolve noun anchors from the DB, prompt an LLM for
exactly one verse of English, normalize, write to GOI_Bible_English/.

This is a REAL inference-driven translator — there is no deterministic mapping
from Hebrew morphology to fluent English prose. An LLM call happens per verse.
...
Usage:
  python3 OT_to_English.py --missing-only
  python3 OT_to_English.py --book GEN --chapter 1
  python3 OT_to_English.py --book GEN --chapter 1 --verse 1 --dry-run
"""
...
NIM = ROOT / "Meta_Bible_Data" / "Bible_Noun_Extraction"
...
SOURCE_DIR = ROOT / "Reference_Bible" / "Hebrew_Bible_WLC" / "One_Directory_WLC_KJV"
OUTPUT_DIR = ROOT / "GOI_Bible" / "GOI_Bible_English"

# Canon-order numeric prefix, matching the KJV-addressed WLC filenames exactly —
# this is the "tautological filename structure" the output must follow:
#   {NNN}_{BOOK}_{chapter:03d}_{verse:03d}.txt
BOOK_NUM = {
...
(WLC) Old Testament into English.

You will receive:
1. The pointed Hebrew verse text
2. Noun anchors: each Hebrew lemma in this verse paired with its gloss from the
   project's noun dictionary. NOTE: a gloss is often a comma-separated list of
   KJV options (e.g. "chief, father, principal" for אָב; "angels, God, judges"
   for אֱלֹהִים). It is a menu, NOT a fixed rendering.

Rules:
- Translate from the Hebrew. Do not invent content not present in the source.
- Render EVERY clause and word of the Hebrew. Do not drop, skip, merge, or
  summarize any clause, predicate, name, or number. If the Hebrew has a verb of
  speaking ("וַיֹּאמֶר", and he said), an explicit subject, a number, or a
  repeated phrase, it must appear in the English.
- Preserve explicit negation, number, person, tense/aspect, and argument structure.
- For each noun anchor, choose the option from its gloss list that fits THIS
  context — do not default to the first listed word. In particular אָב is
  normally "father" (not "chief") and אֱלֹהִים is normally "God"/"gods" (not
  "angels") unless the context genuinely requires otherwise.
- Render the divine name (יהוה) as "the LORD" per standard English convention.
- Output exactly one verse, one line, in plain English prose — no verse numbers,
  no commentary, no transliteration, no footnotes, no copying from other version of the bible."""
...
def noun_anchors(cur: sqlite3.Cursor, book_id: int, chapter: int, verse: int) -> list[tuple[str, str]]:
    """(surface_form, english_gloss) for every noun in this verse.

    Reads strongs_ot directly for the source WLC verse(s) underlying the
    KJV-addressed target verse. For merged KJV targets this may span multiple
    source verses."""
    anchors = []
    for src_book_id, src_chapter, src_verse in target_source_refs(
        ...
        rows = cur.execute(
            """
            SELECT s.word, l.definition, l.lemma
            FROM strongs_ot s
            LEFT JOIN strongs_lexicon l ON l.strongs_num = s.strongs_num AND l.language = 'H'
            WHERE s.book_id = ? AND s.chapter = ? AND s.verse = ? AND s.morph LIKE 'N%'
            ORDER BY s.word_pos
            """,
            (src_book_id, src_chapter, src_verse),
        ).fetchall()
        for surface, definition, lemma in rows:
            gloss = definition.strip() if definition else (lemma or surface)
            anchors.append((surface, gloss))
    return anchors


def build_user_prompt(hebrew: str, anchors: list[tuple[str, str]]) -> str:
    lines = [f"Hebrew verse:\n{hebrew}", "", "Noun anchors (Hebrew form -> gloss options; pick the contextually correct one):"]
    if anchors:
...
def main() -> int:
    ...
    if not args.dry_run and (not model or not api_key):
        sys.exit("Set LLM_MODEL/OPENAI_API_KEY at the top of this file (or export "
                 "OPENAI_MODEL / OPENAI_API_KEY) before running.")

    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    targets = iter_targets(cur, args.book, args.chapter, args.verse)
    ...
    for i, (book_code, book_id, chapter, verse) in enumerate(targets, start=1):
        ...
        print(f"  {progress} [....] {ref}: {len(hebrew.split())} Hebrew word(s), "
        ...
        try:
            english = normalize(call_llm(SYSTEM_PROMPT, user_prompt, base_url, model, api_key))
        ...
        print(f"  {progress} [done] {ref} -> {out_path.name} ({elapsed:.1f}s): {english}")

    total_elapsed = time.monotonic() - run_started
    if not args.dry_run:
        print(f"=== {args.book} {args.chapter} complete in {total_elapsed:.1f}s: "
